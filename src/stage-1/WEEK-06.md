...
| 视频范围 | 学习模块 | 学习结果 |
| --- | --- | --- |
| P1-P5 | JavaScript 简介、语句、标识符、变量、脚本引入、注释和输出方式 | 能够在浏览器中引入并运行基础 JavaScript |
| P6-P11 | 数据类型、`typeof` 和算术、赋值、比较、布尔运算符 | 能够使用变量、数据类型和运算符表达计算过程 |
| P12-P19 | `if`、`if...else`、`switch`、三元运算符、`for`、`while`、`break` 和 `continue` | 能够使用条件和循环处理基础流程 |
| P20-P27 | 字符串基础和常用字符串方法 | 能够完成字符串读取、查找、截取、清理和拆分 |
| P28-P36 | 数组、数组遍历和常用数组方法 | 能够使用数组组织数据并完成基础增删查改 |
| P37-P40 | 函数、对象、`Math` 对象和 `Date` 对象 | 能够拆分函数、组织对象数据并使用常用内置对象 |

控制台版 2048 会用到二维数组、纯函数、复制数组、随机数和函数拆分，这些内容需要在视频基础上通过本周练习补足；视频中的 `substr()` 了解历史用法即可，实际练习优先使用 `slice()` 或 `substring()`。
